...
#--------------------------------------------------------------------------------
	...
	for($i=0;$i<count($fetch);$i++){
		...
		// 価格
		$price = ($fetch[$i]["SELLING_PRICE"])?"<p class=\"price_box01\">価 格".number_format(math_tax($fetch[$i]["SELLING_PRICE"]))."円（税込）</p>":"";
		// 商品番号
		...
		if(!$_POST['status']){
			...
			if(search_file_flg("./product_img/",$fetch[$i]['PRODUCT_ID']."_s.*")){
				$img = search_file_disp("./product_img/",$fetch[$i]['PRODUCT_ID']."_s.*","",2);
			}else{
				$img = "";
			}
		...
		if(!file_exists($img)){
			...
			$image = "<div class=\"img_box01\">
                            <a href=\"{$for_detail}\">
                                <img src=\"".$img."?r=".rand()."\"  alt=\"{$pname}\">
                            </a>
                        </div>";
		}
        ...
        if( ($fetch[$i]["STOCK_QUANTITY"]>0) && ($fetch[$i]["CART_CLOSE_FLG"] == 0) ){
	        $btnRegist = "
                <form method=\"post\" action=\"../regist/\">
                    <input type=\"image\" name=\"\" value=\"カートに入れる\" id=\"\" src=\"./images/btn_03.jpg\" alt=\"カートに入れる\">
		...
		#==============================================================================================
		$table .= "
			<div class=\"style_box01 clearfix\">
                {$image}
                <div class=\"content_box01\">
                    <p class=\"title_box01\">{$pname}</p>
                    {$price}
                    <p>{$content}</p>
                    <ul class=\"btn_box01 clearfix\">
                        <li class=\"pull_left mr20\">
                            <a href=\"{$for_detail}\"><img src=\"./images/btn_01.jpg\" alt=\"商品詳細はこちら\"></a>
                        </li>
                        <li class=\"pull_left\">
                            {$btnRegist}
                        </li>
...
// 前ページへのリンク
$pr = "<li class=\"pull_left\"><a href=\"./?p=".urlencode($prev).$cpram."\"><img src=\"../common_img/btn_prev.jpg\" alt=\"前へ\"></a></li>";
if($p <= 1){
	$pr = "";
}

//次ページリンク
$nx = "<li class=\"pull_right\"><a href=\"./?p=".urlencode($next).$cpram."\"><img src=\"../common_img/btn_next.jpg\" alt=\"次へ\"></a></li>";
if($totalpage <= $p){
	$nx = "";
}
