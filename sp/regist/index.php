...
//		・支払方法：郵便振込と代引き	→	メールを送信して終了
	...
	if ( !isset($_SESSION['setParam']['purchase_flg']) ):
		...
		case 1:
				// ＤＢへデータを登録（失敗時：強制終了）
				include("../../regist/LGC_registDB.php");
		...
		// 銀行振り込み、代引きの場合
				...
				// メール送信処理
				include("../../regist/LGC_sendmail.php");
...
// 入力チェックを行い、入力内容確認画面を表示
	...
	// メイン処理：送信データのチェック
	include("../../regist/LGC_inputChk.php");
	if ( $error_message ){
		...
		include("../../regist/LGC_setAmount.php");
...
//	※カートの中身の追加／削除はここで行う
	...
	include("../../regist/LGC_cartList.php");	// カート操作処理ロジック
	// 買い物カゴの中身をチェック
